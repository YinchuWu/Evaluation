...
def main_plot_bb_on_testset(dataset,model_name,test_info): #dataset:test;model:weights checkpoint;test_info: missfresh-yolo-voc-800
    ...
    for name in img_names:
        ...
        im = cv2.imread(raw_img_path)
        im_pred = im.copy()
        plot_bb(im_pred, pred_boxes, pred_class_ids, pred_scores,thres=0.50,textSize=2,textThickness=4)
        im_pred = cv2.resize(im_pred, (im.shape[1] / 2, im.shape[0] / 2), interpolation=cv2.INTER_CUBIC)
...
def main_plot_bb_one(dataset,model_name,test_info,name):
    predict_results_dir='C:\\Users\\lvsikai\\Desktop\\repositories\\eval_yolo_detection\\results\\predict\\missfresh-yolo-voc-800-0619\\yolo-voc-800_16000\\test'
    raw_img_dir = os.path.join(dataset_dir, 'JPEGImages')
    raw_img_path = os.path.join(raw_img_dir,'%s.jpg'%name)
    im = cv2.imread(raw_img_path)
    ...
    im_pred = im.copy()
    plot_bb(im_pred, pred_boxes, pred_class_ids, pred_scores,thres=0.49,textSize=2,textThickness=4)
    im_pred = cv2.resize(im_pred, (im.shape[1] / 2, im.shape[0] / 2), interpolation=cv2.INTER_CUBIC)
...
def plot_bb_exist():
    dataset_dir1 = 'C:\\Users\\lvsikai\\Desktop\\Yi+\\Missfresh\\4MissFreshSmartShelf_Exp'
    dataset_file = os.path.join(dataset_dir1, 'ImageSets', 'Main', '%s5.txt' % dataset)
    f = open(dataset_file)
    img_names = f.readlines()
    f.close()
...
if __name__ == "__main__":
    ...
    DataSets = [('yolo-voc-800_16000', 'test', 'missfresh-yolo-voc-800-0619')]
    ...
    for ds in DataSets:
        ...
        if dataset_name=='test':
            ...
            main_plot_bb_one(dataset_name,model_name,test_info,name)
        else:
