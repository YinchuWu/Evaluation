# -*- coding: utf-8 -*-
"""
Created on Tue Apr 24 15:05:22 2018
...
def main(args):  
    ...
    for one_file in tqdm(file_list):  
        one_file = one_file.replace('images', 'labels').replace('JPEGImages', 'labels').replace('.png', '.txt').replace('.jpg', '.txt')  
        #print("one_file is:\n",one_file)  
...
if "__main__" == __name__:  
    ...
    parser.add_argument('--engine', '-m', default='sklearn', type=str, choices=['original', 'sklearn', 'sklearn-mini'], help='Method to use')  
  
    args = parser.parse_args(['file_list','--num_clusters','5','--output','../results/anchor.txt','--tol','0.005','--engine','sklearn'])  
